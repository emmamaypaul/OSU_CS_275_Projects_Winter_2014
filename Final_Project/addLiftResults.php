...
if(isset($_POST["liftName"]) && !empty($_POST["liftName"]) && isset($_POST["lodgeList"]) && !empty($_POST["lodgeList"]))
{	//if the variables for the table are set and not empty, then we will prepare, bind, execute, and close
	...
	if(!($tblInpt = $mysqli->prepare("INSERT INTO lift(lid, name) VALUES(?,?)")))
	...
	if(!($tblInpt->bind_param("is", $lodgeId, $liftName)))
...
$addResults =("SELECT lift.name AS 'LiftName', lodge.name AS 'Lodge', resort.name AS 'Resort'
				FROM lift 
...
";
	$table=$table."<tr><td><h2>Lift Name</h2></td><td><h2>Lodge Connected To</h2></td><td><h2>Resort Located</h2></td></tr>";

	while($row = $result->fetch_assoc())
	{
		$table = $table."<tr><td>".$row['LiftName']."</td><td>".$row['Lodge']."</td><td>".$row['Resort']."</td></tr>";
