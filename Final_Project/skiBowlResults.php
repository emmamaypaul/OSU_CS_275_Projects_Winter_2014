...
if($result = $mysqli->query($rQuery)) //run query 
{
	...
	while($row = $result->fetch_assoc()) //fetches row info from query 
	{
		$table = $table."<tr><td>".$row['Runs']."</tr></td>";
	}
	$table = $table."</table>";

	echo "<h1>Based on the lift and skill level you chose, these are the runs you should take: </h1>";
	echo $table;
...
<br><br>
<form action="http://web.engr.oregonstate.edu/~paule/CS%20275/skiBowl.php" method="POST">
