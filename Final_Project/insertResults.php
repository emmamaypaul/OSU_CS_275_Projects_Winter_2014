...
if(isset($_POST["skillName"]) && !empty($_POST["skillName"]))
{	//if the variables for the table are set and not empty, then we will prepare, bind, execute, and close
	
	$skillName = $_POST['skillName'];

	$query = "SELECT experience.name FROM experience WHERE name=?";
	if ($stmt = $mysqli->prepare($query))
	{
		...
		if ($numRows != 0)
			echo "Skill level already exists. Please add a unique skill level.<br><br>";
	...
	if(!($tblInpt = $mysqli->prepare("INSERT INTO experience(name) VALUES(?)")))
...
$addResults = sprintf("SELECT experience.name AS Exp FROM experience");
...
";
	$table=$table."<tr><td><h3>Skill Levels</h3></td></tr>";

	while($row = $result->fetch_assoc())
	{
		$table = $table."<tr><td>".$row['Exp']."</td></tr>";
